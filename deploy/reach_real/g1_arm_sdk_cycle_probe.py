#!/usr/bin/env python3
# 作用：通过 rt/arm_sdk 对指定单关节做循环测试，用于确认关节方向、幅度和安全响应。
...
class OfficialStyleProbe:
    def __init__(self, args):
        ...
        if args.joint in ("right_elbow", "right_shoulder_pitch"):
            if abs(args.delta) > 0.50:
                raise ValueError("right_elbow / right_shoulder_pitch 的 delta 限制为 |delta| <= 0.50 rad")
        else:
            if abs(args.delta) > 0.10:
                raise ValueError("其它关节 delta 限制为 |delta| <= 0.10 rad")
...
"""
    ...
    def run(self):
        ...
        return_time = self.args.return_time
        post_hold = 1.0
        total_time = (
            self.args.pre_hold
            + self.args.ramp_time
        ...
        print("[开始] cycle probe：先接管，摆动，再慢慢回原位，最后释放")
        print(
            f"[阶段] pre_hold={self.args.pre_hold:.2f}s, "
            f"ramp_up={self.args.ramp_time:.2f}s, "
            f"hold={self.args.hold_time:.2f}s, "
            f"ramp_down={return_time:.2f}s, "
        ...
        try:
            while True:
                ...
                t1 = self.args.pre_hold
                t2 = t1 + self.args.ramp_time
                t3 = t2 + self.args.hold_time
                t4 = t3 + return_time
                ...
                if elapsed < t1:
                    ...
                    phase = "ramp_up"
                    ratio = (elapsed - t1) / self.args.ramp_time
                    ...
                    phase = "ramp_down"
                    ratio = 1.0 - (elapsed - t3) / return_time
                    ratio = min(1.0, max(0.0, ratio))
                else:
                    phase = "post_hold"
                    ratio = 0.0
            ...
            print("[平滑释放] 已回到初始附近，再 enable=0")
            q_safe = dict(q0)
            for _ in range(50):
                self.write_cmd(pub, q_safe, enable=1.0)
                time.sleep(dt)
            for _ in range(30):
                self.write_cmd(pub, q_safe, enable=0.0)
                time.sleep(dt)
...
def main():
    ...
    parser.add_argument("--return-time", type=float, default=6.0)
