...
class OfficialStyleProbe:
    def __init__(self, args):
        ...
        if args.joint == "right_elbow":
            if abs(args.delta) > 0.25:
                raise ValueError("right_elbow 的 delta 限制为 |delta| <= 0.25 rad")
        else:
            if abs(args.delta) > 0.10:
                raise ValueError("非 right_elbow 关节 delta 限制为 |delta| <= 0.10 rad")
...
"""
    ...
    def run(self):
        ...
        t0 = time.monotonic()
        count = 0
        total_time = self.args.pre_hold + self.args.ramp_time + self.args.hold_time

        print("[开始] takeover probe：先预接管 hold，再发送 5° 小动作")
        print(f"[阶段] pre_hold={self.args.pre_hold:.2f}s, ramp_time={self.args.ramp_time:.2f}s, hold_time={self.args.hold_time:.2f}s")
        ...
        try:
            while True:
                ...
                if elapsed < self.args.pre_hold:
                    phase = "pre_hold"
                    ratio = 0.0
                elif elapsed < self.args.pre_hold + self.args.ramp_time:
                    phase = "ramp"
                    ratio = (elapsed - self.args.pre_hold) / self.args.ramp_time
